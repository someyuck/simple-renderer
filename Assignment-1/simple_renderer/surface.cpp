...
std::vector<Surface> createSurfaces(std::string pathToObj, bool isLight, uint32_t shapeIdx)
{
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        if(intersectionVariant >= 1) surf.boundingBox = constructAABB(surf.vertices);
        if(intersectionVariant == 3) {
            surf.bvh = objBVH(surf.indices.size());
            surf.bvh.constructBVHonTriangles(surf.vertices, surf.indices);
        }
...
Interaction Surface::rayObjBVHInteraction(Ray ray, int nodeIndex)
{
    Interaction siFinal;
    if(rayAABBIntersect(ray, this->bvh.bvhList[nodeIndex].boundingBox) == false) return siFinal;

    if(this->bvh.bvhList[nodeIndex].isLeaf() == true) {
        // hit a leaf, so intersect with its triangles now
        int firstTriangleIndex = this->bvh.bvhList[nodeIndex].firstTriangle;
        for(int i = 0; i < this->bvh.bvhList[nodeIndex].numTriangles; i++)
        {
            int triangleIndex = this->bvh.triangleIndices[firstTriangleIndex + i];
            Vector3f p1 = this->vertices[this->indices[triangleIndex].x];
            Vector3f p2 = this->vertices[this->indices[triangleIndex].y];
            Vector3f p3 = this->vertices[this->indices[triangleIndex].z];

            Vector3f n1 = this->normals[this->indices[triangleIndex].x];
            Vector3f n2 = this->normals[this->indices[triangleIndex].y];
            Vector3f n3 = this->normals[this->indices[triangleIndex].z];
            Vector3f n = Normalize(n1 + n2 + n3);

            Interaction si = Surface::rayTriangleIntersect(ray, p1, p2, p3, n);
            if (si.t <= t_min && si.didIntersect) {
                siFinal = si;
                t_min = si.t;
                ray.t = si.t;
            }
    ...
    int left = this->bvh.bvhList[nodeIndex].leftChildIndex, right = this->bvh.bvhList[nodeIndex].rightChildIndex;
    Interaction siLeft = rayObjBVHInteraction(ray, left);
    Interaction siRight = rayObjBVHInteraction(ray, right);

    Interaction siMin = siLeft;
    if(siRight.t <= siLeft.t && siRight.didIntersect) siMin = siRight;

    if (siMin.t <= ray.t && siMin.didIntersect) {
        siFinal = siMin;
        t_min = siMin.t;
        ray.t = siMin.t;
    }
...
Interaction Surface::rayIntersect(Ray ray)
{
    ...
    {
        t_min = ray.t;
        return rayObjBVHInteraction(ray, 0);
    }
