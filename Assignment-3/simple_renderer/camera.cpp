...
Ray Camera::generateRay(int x, int y, float epsilon_x, float epsilon_y) {
    // shift epsilons to [-0.5, 0.)
    epsilon_x -= 0.5;
    epsilon_y -= 0.5;

    Vector3f pixelCenter = this->upperLeft + (0.5 - epsilon_x) * this->pixelDeltaU + (0.5 - epsilon_y) * this->pixelDeltaV;
