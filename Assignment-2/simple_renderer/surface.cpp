...
void Surface::intersectBVH(uint32_t nodeIdx, Ray& ray, Interaction& si)
{
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            v1 = this->tris[this->getIdx(i + node.firstPrim)].v1;
            v2 = this->tris[this->getIdx(i + node.firstPrim)].v2;
            v3 = this->tris[this->getIdx(i + node.firstPrim)].v3;

            Interaction siIntermediate = this->rayTriangleIntersect(ray, v1, v2, v3, this->tris[this->getIdx(i + node.firstPrim)].normal);
            if (siIntermediate.t <= ray.t && siIntermediate.didIntersect) {
                si = siIntermediate;
                ray.t = si.t;

                // get UV coords using barycentric coords
                float area = Cross((v2 - v1), (v3 - v1)).Length();
                float area_a = Cross((v2 - si.p), (v3 - si.p)).Length(); // opp v1
                float area_b = Cross((v3 - si.p), (v1 - si.p)).Length(); // opp v2
                float area_c = Cross((v1 - si.p), (v2 - si.p)).Length(); // opp v3
                ...
                uv1 = this->tris[this->getIdx(i + node.firstPrim)].uv1;
                uv2 = this->tris[this->getIdx(i + node.firstPrim)].uv2;
                uv3 = this->tris[this->getIdx(i + node.firstPrim)].uv3;

                si.uv = a * uv1 + b * uv2 + c * uv3;
            }
        }
    }
    else {
...
Interaction Surface::rayIntersect(Ray& ray)
{
    ...
    if(si.didIntersect == true) {
        if(this->hasDiffuseTexture() == true) {
            si.hasDiffuse = true;
            si.texturePtr = &this->diffuseTexture;
        }
        else {
            si.hasDiffuse = false;
            si.texturePtr = &this->diffuse;
        }
    } 
